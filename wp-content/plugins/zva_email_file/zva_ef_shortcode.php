...
function zva_ef_shortcode($args) {
    ...
    if($file) {
        ...
        if($url) {
            ...
            $zva_ef_modal_template = ob_get_clean();
            
            $result = '<a href="javascript:void(0)" data-modal-id="modal-' . $post->ID . '" class="zva_ef_shortcode_link" data-file-id="' . $args['id'] . '">' . $args['text'] . '</a>' . $zva_ef_modal_template;
            return $result;
        }
    }
}
